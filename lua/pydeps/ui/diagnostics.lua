...
local function compute_diagnostics(bufnr, deps, resolved, opts)
  ...
  for _, dep in ipairs(deps or {}) do
    ...
    if view.base_class == "lock_mismatch" and view.base_pinned_version then
      table.insert(
        diagnostics,
        make_diag(
          dep,
          "lock mismatch: pinned " .. view.base_pinned_version .. " but resolved " .. resolved_version,
    ...
    if ok_pypi then
      ...
      if view.class == "pin_not_found" and view.pinned_version then
        table.insert(
          diagnostics,
          make_diag(
            dep,
            "pinned version " .. view.pinned_version .. " not found on public PyPI",
            config.options.diagnostic_severity.lock
          )
        )
      end
